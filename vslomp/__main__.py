...
from vslomp.display.imager import ImagerCmd, ImagerCmdProc, ConvertData
...
async def main():
    ...
    async def icp_handler(hmsg: MsgHandle[ImagerCmd], img: Optional[Any]):
        ...
        if isinstance(img, Image):
            if hmsg.cmd == ImagerCmd.LOAD_FILE:
                data = ConvertData(img, "1", FLOYDSTEINBERG)
                await icp.send(ImagerCmd.CONVERT, data)
            else:
                await scp.send(ScreenCmd.DISPLAY, img, 100)
        else:
    ...
    #await icp.send(ImagerCmd.GET_DEMO)
    await icp.send(ImagerCmd.LOAD_FILE, "/home/pi/images/001.jpg")

    await asyncio.gather(scp.join(), icp.join())

    await scp.send(ScreenCmd.SLEEP, pri=190)
    await scp.send(ScreenCmd.UNINIT, pri=200)
